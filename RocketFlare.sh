...
display_welcome_message() {
    clear
    echo -e "\033[0;32m  ______              _               _______ _                   "
    echo -e "\033[0;32m (_____ \            | |         _   (_______) |                  "
    echo -e "\033[0;32m  _____) ) ___   ____| |  _ ____| |_  _____  | | ____  ____ ____  "
    echo -e "\033[0;32m (_____ ( / _ \ / ___) | / ) _  )  _)|  ___) | |/ _  |/ ___) _  ) "
    echo -e "\033[0;32m       | | |_| ( (___| |< ( (/ /| |__| |     | ( ( | | |  ( (/ /  "
    echo -e "\033[0;32m       |_|\___/ \____)_| \_)____)\___)_|     |_|\_||_|_|   \____) "
    echo -e "\033[0m"                                                                
...
check_rocket_loader_injection() {
    local base_url="$1"
    local vuln_path="/cdn-cgi/image/width=1000,format=auto/https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/cloudflare.svg"
    ...
    if [[ "$response" -eq 200 ]]; then
        ...
        if echo "$body" | grep -q "Cloudflare" && echo "$body" | grep -q "<svg" && echo "$body" | grep -q "M16.5088 16.8447c.1475"; then
...
main() {
    ...
    echo -e "\033[1;34m[i] Starting Rocket Loader HTML Injection check...\033[0m"
    echo ""
    ...
    if [ "$urls_from_file" = true ]; then
        while IFS= read -r base_url || [ -n "$base_url" ]; do
            ...
            if is_valid_url "$base_url"; then
                ((total_scanned++))
                check_rocket_loader_injection "$base_url"
